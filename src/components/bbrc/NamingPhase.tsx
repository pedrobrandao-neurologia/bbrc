...
import { matchTargetFigures, PHASE_INSTRUCTIONS } from '@/lib/bbrc-constants';

interface NamingPhaseProps {
  onComplete: (score: number, transcript: string) => void;
}

export function NamingPhase({ onComplete }: NamingPhaseProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    speakText(PHASE_INSTRUCTIONS.naming)
    ...
    <div className="flex min-h-screen flex-col items-center bg-background p-4">
      <Card className="w-full max-w-4xl">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl">1. Nomeação</CardTitle>
          <p className="text-muted-foreground">Diga o nome de cada figura</p>
        </CardHeader>
